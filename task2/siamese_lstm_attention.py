...
class SiameseBiLSTMAttention(nn.Module):
    def __init__(
        ...
        self.TransformerEncoder = Transformer(vocab_size, self.embedding_size, self.encoder_layers)
    ...
    def forward_once(self, batch, lengths):
        ...
        # we creating mask for transformer encoder
        mask = torch.randn(embeddings.size(0), embeddings.size(1)).ge(0)
        tr_enc = self.TransformerEncoder(embeddings, mask)
